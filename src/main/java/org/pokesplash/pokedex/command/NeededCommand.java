    ...
    public int run(CommandContext<ServerCommandSource> context) {
        try {
            ...
            for (DexEntry entry : caught) {
                Species sp = PokemonSpecies.INSTANCE.getByPokedexNumber(entry.getDexNumber(), Cobblemon.MODID);
                if(sp != null){
                    if (PokeDex.config.isImplementedOnly()) {
                        if (sp.getImplemented()) {
                            species.add(sp);
                        }
                    } else {
                        species.add(sp);
                    ...
                    String message = String.format("Species null DexNum (%s)", entry.getDexNumber());
